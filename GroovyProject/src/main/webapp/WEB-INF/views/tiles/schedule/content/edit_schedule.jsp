	...
	$(document).ready(function(){
		...
		$.ajax({
			...
			data: {"fk_lgcatgono":"${requestScope.map.FK_LGCATGONO}", 
				   "empno":"${requestScope.map.FK_EMPNO}"},
			dataType: "json",
			async: false,  //동기방식
		...
		// 일정소분류 종류를 알아와서 select 태그에 넣어주기 
		...
		// 	console.log("색상 => " + $("input#color").val());
			...
			frm.action="<%= ctxPath%>/schedule/editScheduleEnd.on";
			frm.method="post";
			frm.submit();
...
<div style="margin: 0 auto; width:95%;">
	...
	<div class="" id="insert_schedule" style="width:95%; margin-left: 2.5%;">
		...
		<form name="scheduleFrm">
			...
			<table style="width:100%;">
				<tr class="insert_sche_tr">
					...
					<td class="col-10">
						<input class="input_width" type="text" id="subject" name="subject" value="${requestScope.map.SUBJECT}">
					</td>
				</tr>
				...
				<tr class="insert_sche_tr">
					<th class="col-2"><label class="mr-5 insert_sche_title" >색상</label></th>
					<td class="col-10">
						<input class="" type="color" id="color" name="color" list="list" value="${requestScope.map.COLOR}"/>
						<datalist id="list">
				...
				<tr class="insert_sche_tr">
					...
					<td class="col-10">
						<input class="input_width" type="text" id="place" name="place" value="${requestScope.map.PLACE}">
					</td>
				</tr>
		...
		<div style="float:right;" class="mr-2 mt-4">
			<button class="btn bg-light mr-2" onclick="javascript:location.href='<%= ctxPath%>/schedule/viewSchedule.on?scheduleno=${requestScope.map.SCHEDULENO}'">취소</button>
