...
const goSearch = () => {
	...
	frm.method = "get";
	frm.action = "<%=ctxPath%>/approval/personal/processed.on";
	frm.submit();
}
...
const excelDownLoad = () => {
	...
	downloadArray = Array.from($("#processedDraftTable > tbody > tr").children());
...
<div id='list' class='m-4'>
	...
	<form name="searchFrm">
 	...
 	<form name="excelFrm">
		...
		<input type="hidden" name="listName" value="개인문서함-결재함" />
		<input type="hidden" name="header" value="결재완료일,기안일,종류,문서번호,제목,기안자,결재상태" />
	</form>		

	<table class="table" id="processedDraftTable">
		<thead>
			<tr class='row'>
